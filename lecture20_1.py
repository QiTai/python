...
##    hist(diffs)
##    axvline(diffMean, color = 'y', label = 'Mean')
##    legend()
##    titleString = str(numFlips) + ' Flips, ' + str(numTrials) + 'Trails' 
##    title(titleString)
##    xlabel('Difference between heads and tails')
##    ylabel('Number of Trails')
##    figure()   ##to generate a new figure
##    plot(diffPercent)
...
def throwDarts(numDarts, shouldPlot):
    ...
    for darts in range(1, numDarts + 1, 1):
        ...
        if darts%1000000 == 0 : # so I know it's making progress
            piGuess = 4*(inCircle/float(darts))
            dartStr = locale.format('%d', darts, True)
            print('Estimat with', formatInt(darts), 'darts:', piGuess)
    if shouldPlot:
        xAxis = arange(1, len(estimates)+1)
        semilogx(xAxis, estimates)
        titleString = 'Estimate of pi, final estimate: ' + str(piGuess)
        title(titleString)
