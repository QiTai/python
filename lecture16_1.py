class Person(object):
    ...
    def __str__(self):
        return '<Person: %s %s>'%(self.first_name, self.family_name)  #注意这种新的返回方式
...
class MITPerson(Person):                ##继承
    nextIdNum = 0                       ##类里面的静态变量，有点像static.
    def __init__(self, familyName, firstName):
        Person.__init__(self, familyName, firstName)
        self.idNum = MITPerson.nextIdNum
        MITPerson.nextIdNum += 1         ##下次创建MITPerson,nextIdNum保留
    def getIdNum(self):
        return self.idNum
    def __str__(self):
